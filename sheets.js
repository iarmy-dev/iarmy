  ...
  try {
    ...
    for (let row = 2; row <= 32; row++) {
      formulas.push([
        ...
        `=IF(OR(F${row}="",K${row}=""),"",F${row}-K${row})`,
        // M: Cumul = Cumul précédent + Non Décl
        row === 2 
          ? `=IF(L${row}="","",L${row})` 
          : `=IF(L${row}="","",IF(M${row-1}="",L${row},M${row-1}+L${row}))`
      ]);
    }
    ...
    // 5. Ajouter ligne TOTAL en bas (ligne 34)
    await sheets.spreadsheets.values.update({
      spreadsheetId: SPREADSHEET_ID,
...
module.exports = { writeToSheet, getSheetName };
